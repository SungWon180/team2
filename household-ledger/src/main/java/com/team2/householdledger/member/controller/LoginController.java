...
public class LoginController {
    ...
    @PostMapping("/login")
    public CommonDTO<UserDTO> login(@RequestBody UserDTO loginRequest, HttpSession session) {
        UserDTO user = userService.login(loginRequest.getUserId(), loginRequest.getUserPw());
        if (user != null) {
            session.setAttribute("loginUser", user);
            return CommonDTO.<UserDTO>success(user);
        } else {
